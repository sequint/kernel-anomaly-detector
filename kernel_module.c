...
{
    // Set a task struct pointer to use for each process
    ...
    for_each_process(task)
    {
        ...
        // If either exceed predefined thresholds, log the process
        if (cpu_usage > CPU_THRESHOLD || mem_usage > MEM_THRESHOLD)
        {
            printk(KERN_INFO "Flagged Anomoly Process: %s, PID: %d, CPU: %lu, Memory: %lu MB\n", task->comm, task->pid, cpu_usage, mem_usage);
            anomaly_found = true;
        }
    }
    ...
    if (!anomaly_found)
    {
        printk(KERN_INFO "No anomoly processes found\n");
    }
}
